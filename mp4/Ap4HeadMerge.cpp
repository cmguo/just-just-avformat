...
namespace ppbox
{
    namespace avformat
    {
        ...
        error_code Ap4HeadMerge::CheckMp4Head(error_code & ec)
        {
            try
            {
        ...
        error_code Ap4HeadMerge::WriteToMemStream(char const * str, boost::int32_t n)
        {
            stream_->Seek(end_);
        ...
        error_code Ap4HeadMerge::WriteHeadToMemStream(error_code & ec)
        {
        ...
        AP4_LargeSize Ap4HeadMerge::GetHeadStreamSize()
        {
            AP4_Position end = 0;
        ...
        boost::uint64_t Ap4HeadMerge::GetHeadSize()
        {
            return head_size_;
        }

        boost::uint64_t Ap4HeadMerge::GetContentLength()
        {
            return content_length_;
        }

        boost::uint64_t Ap4HeadMerge::GetContentSize()
        {
            return content_size_;
        }

        error_code Ap4HeadMerge::ReadFromHeadStream(void * p, AP4_Size size)
        {
            error_code ec;
            head_stream_->Seek(0);
        ...
        error_code Ap4HeadMerge::SeekHeadStream(AP4_Position position)
        {
            error_code ec;
        ...
        error_code Ap4HeadMerge::CompareStsdDescription(AP4_Track *cur_track, AP4_Track *source_track)
        {
            error_code ec;
        ...
        error_code Ap4HeadMerge::FindTrack(Ap4HeadMerge & mp4_head,
                                          AP4_Track *cur_track,
                                          AP4_Track **p)
        {
            error_code ec;
        ...
        AP4_Atom* Ap4HeadMerge::FindTrackChild(AP4_Track * track, char const * path)
        {
            return track->GetTrakAtom()->FindChild(path);
        }

        error_code Ap4HeadMerge::MergeTrack(Ap4HeadMerge &mp4_head,
                                   AP4_Track *cur_track,
        ...
        error_code Ap4HeadMerge::Merge(Ap4HeadMerge & mp4_head)
        {
            error_code ec;
        ...
        AP4_Size Ap4HeadMerge::CaclHeadSize()
        {
            AP4_Size nSize=0;
            AP4_Cardinal n = file_->GetTopLevelAtoms().ItemCount();
            AP4_List<AP4_Atom>::Item *item = file_->GetTopLevelAtoms().FirstItem();
        ...
        void Ap4HeadMerge::SetContentLength(AP4_Size contentlength)
        {
            content_length_ = contentlength;
        }

        void Ap4HeadMerge::SetMemStreamEndPosition(AP4_Position pos)
        {
            end_ = pos;
        }

        AP4_Atom* Ap4HeadMerge::FindChild(const char *path)
        {
            return file_->FindChild(path);
        }

        void Ap4HeadMerge::Clean()
        {
            if (file_)
            {
                delete file_;
