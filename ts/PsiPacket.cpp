...
namespace ppbox
{
    namespace avformat
    {
        ...
        PatPacket::PatPacket()
            ...
            , table(PsiTableId::program_association, 0)
        {
            sections.push_back(PatSection(default_program_number, TsPid::pmt));
            table.section_length += 4;
            fill_payload2(table.section_size());
        }

        PmtPacket::PmtPacket()
            : TsPacket(TsPid::pmt, 0)
            , table(PsiTableId::program_map, 4)
            , byte12(0xe000)
            , byte34(0xf000)
        {
            table.transport_stream_id = default_program_number; // transport_stream_id is program_number for pmt
        }

        void PmtPacket::add_stream(
            bool video, 
            boost::uint8_t type, 
            boost::uint32_t index)
        {
            ...
            streams.push_back(TsStreamInfo(type, TsPid::stream_base + (boost::uint16_t)index));
            table.section_length += 5;
        }
